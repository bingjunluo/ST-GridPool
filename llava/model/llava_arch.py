...
from prepare_inputs import SoftmaxWeightedL2NormPool2d, create_image_grid
from einops import rearrange
import os

def prepare_unit(image_features, grid_size_list=[], grid_freq_list=[], kernel_size=2, padding=0, temp=1., p=1):
    if len(grid_size_list) > 0:
        ...
        for feature_grid_size, feature_grid_freq in zip(grid_size_list, grid_freq_list):
            ...
            for i in range(num_grids):
                ...
                image_features[grid_frame_inds[start_idx + feature_grid_size - 1].item()] = img.unsqueeze(0)
        
        # for ind in update_frame:
        #     image_features[ind] = update_frame[ind]

    pool = SoftmaxWeightedL2NormPool2d(kernel_size=(kernel_size, kernel_size), padding=padding, temp=temp, p=p)
    pooled_image_features = pool(image_features)
    return pooled_image_features
...
class LlavaMetaForCausalLM(ABC):
    ...
    def get_2dPool(self, image_feature, stride=2):
        ...
        if self.config.mm_spatial_pool_mode == "average":
            ...
            # image_feature = nn.functional.avg_pool2d(image_feature, stride, padding=1)
            
            # pool = SoftmaxWeightedL2NormPool2d(kernel_size=(2, 2), stride=(2, 2), padding=0, temp=1, p=1)
            # image_feature = pool(image_feature)

            image_feature = eval(f"prepare_unit(image_feature, {os.environ['PREPARE_CONFIG']})")
